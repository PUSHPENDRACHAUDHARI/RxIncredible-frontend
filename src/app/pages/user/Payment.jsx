...
import { CheckCircle, CreditCard, IndianRupee, FileText, Stethoscope, Pill, ArrowLeft, Loader2, XCircle, AlertTriangle } from "lucide-react";
import Swal from "sweetalert2";
import { Navbar } from "../../components/Navbar.jsx";
import { Footer } from "../../components/Footer.jsx";
import { orderAPI, paymentAPI, documentAPI, authAPI, getToken } from "@/services/api.js";
import {
...
const getNormalizedServiceFee = (order, country) => {
  ...
  const expectedTotal = shouldApplyGstForCountry(country) &&
      (serviceType === "PRESCRIPTION_ANALYSIS" || serviceType === "SECOND_OPINION")
...
export default function UserPayment() {
  ...
  const getInitialPricingCountry = () => {
    ...
    const stateCountry = stateOrder?.deliveryCountry || stateOrder?.user?.country;
    if (stateCountry) {
      return stateCountry;
    }
    ...
    try {
      ...
      console.warn("Unable to read pending order country from session storage", error);
    }
  ...
  console.log('Payment component - serviceId:', serviceId, 'cleanOrderId:', cleanOrderId, 'isPaymentLink:', isPaymentLink);
  ...
  if (isPaymentLink && orderIdFromUrl) {
    ...
    if (!token) {
      ...
      sessionStorage.setItem('redirectAfterLogin', '/user/pay/' + orderIdFromUrl);
  ...
  const generateAndDownloadReceipt = async (orderId) => {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 5; attempt += 1) {
        try {
          console.log(`Downloading generated receipt for order ${orderId}, attempt ${attempt}`);
          const downloadResponse = await orderAPI.downloadReceipt(orderId);

          if (!downloadResponse.ok) {
            throw new Error(`Receipt download response failed with status ${downloadResponse.status}`);
          }
          ...
          console.warn(`Receipt download attempt ${attempt} failed:`, downloadError);
          if (attempt < 5) {
            await wait(1200);
          }
        }
      ...
      console.warn("Receipt download failed after retries, trying bill download fallback");
      const billResponse = await orderAPI.downloadBill(orderId);
      if (!billResponse.ok) {
        throw lastReceiptError || new Error("Receipt and bill download both failed");
      }
  ...
  const fetchExistingOrder = async (orderId, userObj = null) => {
    ...
    try {
      ...
      setPricingCountry(order.deliveryCountry || order.user?.country || user?.country || "India");
  ...
  useEffect(() => {
    ...
    if (stateOrder) {
      ...
      sessionStorage.setItem('pendingOrderForPayment', JSON.stringify(stateOrder));
      setLoading(false);
    }
  }, [location.state]);
  ...
  useEffect(() => {
    console.log('=== Main useEffect START ===', { serviceId, orderIdFromUrl, isPaymentLink, pathname: location.pathname });
    ...
    if (serviceId && !isNaN(serviceId) && parseInt(serviceId) > 0 && !isPaymentLink) {
      // This is an existing order - verify user is logged in first
      const token = getToken();
      if (!token) {
        sessionStorage.setItem('redirectAfterLogin', '/user/payment/' + serviceId);
        Swal.fire({
          icon: "warning",
    ...
    if (!existingOrder && !orderIdFromUrl && !isPaymentLink && !pricingCountry) {
      return;
    }
    ...
    setLoading(false);
  }, [navigate, serviceId, orderIdFromUrl, isPaymentLink, location.pathname, pricingCountry]);
  ...
  const shouldApplyGst = () => {
    const country =
      existingOrder?.deliveryCountry || existingOrder?.user?.country || pricingCountry;
    ...
    const hasOnlinePharmacyOnly =
      (serviceData?.services?.onlinePharmacy || serviceId === "online-pharmacy") &&
      !hasPrescriptionAnalysis &&
      !hasSecondOpinion;

    return !hasOnlinePharmacyOnly && (hasPrescriptionAnalysis || hasSecondOpinion);
  };
  ...
  const handlePayment = async (e) => {
    ...
    try {
      ...
      if (!Number(total) || Number(total) <= 0) {
        throw new Error("A payable amount is required before starting payment.");
      }
      ...
      if (!orderForPayment) {
        ...
        setExistingOrder(orderForPayment);
      ...
      const razorpayConfig = razorpayConfigResponse.data || razorpayConfigResponse;
      ...
      const razorpayOrderResponse = await paymentAPI.createRazorpayOrder({
        ...
        currency: shouldApplyGstForCountry(existingOrder?.deliveryCountry || existingOrder?.user?.country || pricingCountry) ? "INR" : "USD",
      });
      ...
      Swal.fire({
        ...
        title: isUserCancelled ? "Payment Cancelled" : isNetworkError ? "Server Unavailable" : "Payment Failed",
          ...
          <div className="max-w-2xl mx-auto">
            ...
            >
              ...
              <p className="text-gray-600 mb-8">
                You are about to reject the bill for order #{orderIdFromUrl}. This action cannot be undone.
              </p>
    ...
    <>
      ...
      <main className="flex-1 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          ...
          >
            <h1 className="text-4xl text-[#1E3A8A] mb-4">Payment</h1>
            <p className="text-xl text-gray-600">Complete your payment to proceed</p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <form onSubmit={handlePayment} className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      >
                        <CreditCard className="w-6 h-6 mx-auto mb-2 text-[#2563EB]" />
                      ...
                      >
                        <IndianRupee className="w-6 h-6 mx-auto mb-2 text-[#2563EB]" />
                        <span className="text-sm text-gray-700">UPI/Wallet</span>
                      </button>
                    </div>
                  </div>
                  ...
                  {paymentMethod === "card" && (
                    <motion.div
                      ...
                      className="rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm text-gray-700"
                    >
                      Card details will be entered in the secure Razorpay checkout. As soon as you click submit, the official payment window will open.
                    </motion.div>
                  )}
                  ...
                  {paymentMethod === "upi" && (
                    <motion.div
                      ...
                      className="rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm text-gray-700"
                    >
                     UPI app, UPI ID, or wallet selection will be handled in the secure Razorpay checkout. As soon as you click submit, the payment window will open.
                    </motion.div>
                  )}
                  ...
                  >
                    {processing ? (
                      ...
                      <>
                        ...
                        Pay {formatCurrencyAmount(total, pricingCountry)}
                      </>
                    )}
                  </motion.button>
                </form>
            ...
            >
              <div className="bg-white rounded-3xl shadow-xl p-6 sticky top-24">
                <h3 className="text-xl text-[#1E3A8A] mb-6">Order Summary</h3>
                ...
                {existingOrder ? (
                  <div className="space-y-4">
                    <div className="p-4 bg-blue-50 rounded-xl">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="font-medium text-gray-700">Order Details</span>
                      </div>
                      <p className="text-sm text-gray-600">Order #{existingOrder.orderNumber}</p>
                      <p className="text-sm text-gray-600">Service: {existingOrder.serviceType?.replace('_', ' ')}</p>
                      <p className="text-sm text-gray-600">Status: {existingOrder.status}</p>
                    </div>
                    
                    <div className="space-y-3">
                      <div className="flex justify-between text-gray-700">
                        <span>Service Fee</span>
                        <span>{formatCurrencyAmount(existingOrder.totalAmount || 0, pricingCountry)}</span>
                      </div>
                      {gstRate > 0 && (
                        <div className="flex justify-between text-gray-700">
                          <span>GST ({gstRate}%)</span>
                          <span>{formatCurrencyAmount(gst, pricingCountry)}</span>
                        </div>
                      )}
                      <div className="border-t border-gray-200 pt-4">
                        <div className="flex justify-between text-[#1E3A8A]">
                          <span>Total Amount</span>
                          <span className="text-2xl">{formatCurrencyAmount(total, pricingCountry)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ) : serviceData?.services?.prescriptionAnalysis && serviceData?.services?.secondOpinion ? (
                  <div className="flex items-center gap-3 p-3 bg-gradient-to-r from-[#16A34A] to-[#22C55E] rounded-xl text-white">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs green-100">Prescription Analysis + Second Opinion</p>
                    </div>
                  </div>
                ) : (
                  <>
                    {serviceData?.services?.prescriptionAnalysis && (
                      <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-xl">
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-medium text-gray-700">Prescription Analysis</p>
                          <p className="text-xs text-gray-500">Document review & analysis</p>
                        </div>
                        <span className="text-sm font-medium text-[#1E3A8A]">
                          {formatCurrencyAmount(servicePricing["prescription-analysis"].price, pricingCountry)}
                        </span>
                      </div>
                    )}
                    ...
                    {serviceData?.services?.secondOpinion && (
                      <div className="flex items-center gap-3 p-3 bg-green-50 rounded-xl">
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-medium text-gray-700">Second Opinion</p>
                          <p className="text-xs text-gray-500">Expert medical opinion</p>
                        </div>
                        <span className="text-sm font-medium text-[#1E3A8A]">
                          {formatCurrencyAmount(servicePricing["second-opinion"].price, pricingCountry)}
                        </span>
                      </div>
                    )}

                    {(serviceData?.services?.onlinePharmacy || serviceId === 'online-pharmacy') && !serviceData?.services?.prescriptionAnalysis && !serviceData?.services?.secondOpinion && (
                      <div className="flex items-center gap-3 p-3 bg-purple-50 rounded-xl">
                        <Pill className="w-5 h-5 text-[#7C3AED]" />
                        <div className="flex-1">
                          <p className="text-sm font-medium text-gray-700">Online Pharmacy</p>
                          <p className="text-xs text-gray-500">Prescription will be sent to your email</p>
                        </div>
                        <span className="text-sm font-medium text-[#1E3A8A]">
                          Free
                        </span>
                ...
                {!existingOrder && (
                  <>
                    <div className="space-y-4 mb-6">
                      <div className="flex justify-between text-gray-700">
                        <span>Service Fee</span>
                        <span>{formatCurrencyAmount(serviceFee, pricingCountry)}</span>
                      </div>
                      {gstRate > 0 && (
                        <div className="flex justify-between text-gray-700">
                          <span>GST ({gstRate}%)</span>
