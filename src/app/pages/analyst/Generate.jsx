...
const PDF_FILE_NAME = "prescription analysis report.pdf";
const PDF_TITLE = "Prescription Analysis Report";
...
const sanitizeSingleLineMedicalText = (value) =>
  String(value || "")
    .replace(/[^a-zA-Z0-9\s().,+\-/%]/g, "")
    .replace(/\s{2,}/g, " ")
    .trimStart();

const sanitizeMultiLineMedicalText = (value) =>
  String(value || "")
    .replace(/[^a-zA-Z0-9\s.,:/()%+\-'"&\n]/g, "")
    .replace(/[ \t]{2,}/g, " ")
    .replace(/\n{3,}/g, "\n\n");
...
const validateMedicineEntry = (medicine = {}) => {
  ...
  const description = medicine.description?.trim() || "";
  ...
  if (!name) {
    ...
    errors.name = "Enter a valid medicine name using letters, numbers, spaces, and standard medical symbols.";
  }

  if (form && (form.length > 50 || !MEDICINE_FIELD_PATTERN.test(form))) {
    errors.form = "Enter a valid drug form such as tablet, capsule, syrup, or injection.";
  }

  if (strength && (strength.length > 50 || !MEDICINE_FIELD_PATTERN.test(strength))) {
    errors.strength = "Enter a valid strength such as 500 mg or 5 ml.";
  }

  if (frequency && (frequency.length > 80 || !MEDICINE_FIELD_PATTERN.test(frequency))) {
    errors.frequency = "Enter a valid frequency such as once daily or twice daily.";
  }

  if (duration && (duration.length > 50 || !MEDICINE_FIELD_PATTERN.test(duration))) {
    errors.duration = "Enter a valid duration such as 5 days or 2 weeks.";
  }

  if (description) {
    const descriptionError = validateOptionalTextField("Medicine description", description, {
      minLength: 5,
      maxLength: 300,
    });
    if (descriptionError) {
...
export default function AnalystGenerate() {
  ...
  const [medicines, setMedicines] = useState([
    { name: "", form: "", strength: "", frequency: "", duration: "", description: "" }
  ]);
    ...
    const fetchPatientData = async () => {
      ...
      try {
        ...
        if (orderRes.ok) {
          ...
          const allowedServiceTypes = ['PRESCRIPTION_ANALYSIS'];
          if (orderData.serviceType && !allowedServiceTypes.includes(orderData.serviceType)) {
            Swal.fire({
              icon: 'warning',
              title: 'Access Denied',
              text: 'You can only process Prescription Analysis orders.',
  ...
  const addMedicine = () => {
    setMedicines([...medicines, { name: "", form: "", strength: "", frequency: "", duration: "", description: "" }]);
  ...
  const updateMedicine = (index, field, value) => {
    ...
    updated[index] = {
      ...
      [field]:
        field === "description"
          ? sanitizeMultiLineMedicalText(value)
          : sanitizeSingleLineMedicalText(value),
    };
  ...
  const updateOptionalField = (field, setter, value) => {
    setter(sanitizeMultiLineMedicalText(value));
    setFormErrors((prev) => {
      if (!prev[field]) {
        return prev;
      }
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        try {
          ...
          // Use getBlob which returns a Promise - more reliable than callback-based getBase64
  ...
  const generatePDF = async function() {
    ...
    try {
      ...
      // Use download method for direct download
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validationResult.isValid) {
      Swal.fire({
        ...
        text: 'Please correct the highlighted medical information, prescription, or additional information fields before submitting.',
        confirmButtonColor: '#2563EB'
      });
      return;
    }
    ...
    try {
      ...
      const medicinesText = medicines
        .filter(m => m.name)
        .map((m, i) => `${i + 1}. ${m.name.toUpperCase()}${m.form ? ', ' + m.form : ''}${m.strength ? ', ' + m.strength : ''}${m.frequency ? ', ' + m.frequency : ''}${m.duration ? ', ' + m.duration : ''}${m.description ? ', ' + m.description : ''}`)
        .join('\n');
      ...
      const statusResponse = await fetch(
        ...
        buildAuthenticatedRequestOptions({
          ...
          body: JSON.stringify({
            status: "COMPLETED",
            serviceType: originalServiceType || "PRESCRIPTION_ANALYSIS",
      ...
      <main className="flex-1 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <motion.div
            ...
            className="mb-8"
          >
            <h1 className="text-4xl text-[#1E3A8A] mb-4">Generate Medical Report</h1>
            <p className="text-xl text-gray-600">Report ID: {reportId}</p>
          </motion.div>
                  ...
                  <div className="space-y-3">
                    {patientData.documents.map((doc, index) => (
                      ...
                      >
                        ...
                        <Download className="w-5 h-5 shrink-0 text-[#2563EB]" />
                      </motion.div>
                    ))}
                  </div>
                ) : (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#2563EB]"
                          required
                        />
                      </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          ...
                          <div className="md:col-span-2">
                            <textarea
                              ...
                              placeholder="Description: Right time to take, how to take, when medicine will take effect (e.g., Take after meals, twice daily. Medicine will start working within 30 minutes.)"
                              rows={2}
                              className={`${medicineInputClass("description")} resize-none`}
                            />
                            {currentMedicineErrors.description && (
                              <p className="mt-1 text-sm text-red-600">{currentMedicineErrors.description}</p>
                            )}
                          </div>
                        </div>
                            </>
                  ...
                  <div className="flex flex-wrap gap-4">
                    ...
                    <motion.button
                      ...
                      className="px-8 py-4 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                      {savingDraft ? (
                        <>
